...
Future<void> loadDoctorData() async {
  ...
  Future<void> _selectTime(String day, bool isStartTime) async {
    ...
    final initialTime =
        isStartTime
            ? (_workingSchedule[dayIndex]['startTime'] ??
                const TimeOfDay(hour: 9, minute: 0))
            : (_workingSchedule[dayIndex]['endTime'] ??
  ...
  Future<void> _submit() async {
    ...
    if (_hasClinic) {
      final selectedDays =
          _workingSchedule.where((item) => item['isSelected'] == true).toList();
      if (selectedDays.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Please select at least one working day for the clinic',
            ),
          ),
        );
        return;
      ...
      for (var dayItem in selectedDays) {
        if (dayItem['startTime'] == null || dayItem['endTime'] == null) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                'Please set start and end times for ${dayItem['day']}',
              ),
            ),
          );
          return;
    ...
    try {
      ...
      if (_hasClinic) {
        ...
        DocumentReference clinicRef = await FirebaseFirestore.instance
            .collection('clinics')
            .add(clinicData);
            ...
            child: ListView(
              children: [
                ...
                if (_hasClinic) ...[
                  ...
                  const SizedBox(height: 8),
                  ..._daysOfWeek
                      .map((day) => _buildDayTimePicker(day))
                      .toList(),
                ],
  ...
  Widget _buildDayTimePicker(String day) {
    ...
    final dayIndex = _workingSchedule.indexWhere((item) => item['day'] == day);
    final isSelected =
        dayIndex != -1 ? _workingSchedule[dayIndex]['isSelected'] : false;
  ...
  Widget _buildTimePickerButton(String day, bool isStart) {
    ...
    return ElevatedButton.icon(
      ...
      style: ElevatedButton.styleFrom(
        ...
        foregroundColor:
            time != null ? theme.textTheme.bodyLarge?.color : theme.hintColor,
        elevation: 1,
